...
const createMdComponents = (theme: string) => ({
  ...
  code: ({ className, children, inline, ...props }: MdComponentProps & { inline?: boolean }) => {
...
const AiMessageBubble: React.FC<AiMessageBubbleProps> = ({
  ...
  return (
    <div className={`relative break-words flex flex-col`}>
      ...
      <Button
        variant="default"
        className={`cursor-pointer bg-muted border-border text-muted-foreground self-end ${
...
interface ChatMessagesViewProps {
  ...
  onSubmit: (inputValue: string, effort: string, model: string, provider: string, temperature: number) => void;
  onCancel: () => void;
...
export function ChatMessagesView({
  ...
  const mdComponents = createMdComponents(theme);
          ...
          {isLoading &&
            (messages.length === 0 ||
              ...
              <div className="flex items-start gap-3 mt-3">
                ...
                <div className="relative group max-w-[85%] md:max-w-[80%] rounded-xl p-3 shadow-sm break-words bg-card text-card-foreground border border-border dark:bg-neutral-800 light:bg-neutral-100 rounded-bl-none w-full min-h-[56px]">
